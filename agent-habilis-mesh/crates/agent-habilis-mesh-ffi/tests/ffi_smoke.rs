...
use agent_habilis_mesh_ffi::ffi::{
    ...
    mesh_name, mesh_nickname, mesh_open, mesh_peer_count, mesh_peers_json, mesh_recv, mesh_send,
...
fn null_arguments_are_errors_not_crashes() {
    ...
    unsafe {
        ...
        assert!(mesh_name(std::ptr::null()).is_null());
...
/// The mesh name is decoded from the 💬… id itself (not gossiped), so a
/// creator's chosen name is already visible through `mesh_name()` on a joiner
/// the instant `mesh_open` returns — no roster wait needed.
#[test]
fn a_joiner_reads_back_the_creators_mesh_name() {
    ...
    assert!(
        !creator.is_null(),
        "mesh_open (create) failed: {:?}",
        last_error()
    );

    // SAFETY: `creator` is live for the rest of this test; the returned
    // pointers are borrowed from it and read before it is closed.
    let (id, name) = unsafe { (mesh_id(creator), mesh_name(creator)) };
    assert!(!id.is_null(), "mesh_id returned NULL");
    assert!(!name.is_null(), "mesh_name returned NULL");
    // SAFETY: both are non-NULL NUL-terminated strings owned by the handle.
    let (id, name) = unsafe { (CStr::from_ptr(id), CStr::from_ptr(name)) };
    assert_eq!(
        name.to_bytes(),
        b"jam-room",
        "the creator's own mesh_name must echo what it asked for"
    );
    let id = id.to_owned();
    ...
    assert!(
        !joiner.is_null(),
        "mesh_open (join) failed: {:?}",
        last_error()
    );

    // SAFETY: `joiner` is live for the rest of this test.
    let joined_name = unsafe { mesh_name(joiner) };
    assert!(!joined_name.is_null(), "mesh_name (joiner) returned NULL");
    ...
    // SAFETY: each handle came from `mesh_open` and is not used after this.
    unsafe {
        assert_eq!(mesh_close(joiner), 0, "mesh_close (joiner) failed");
        assert_eq!(mesh_close(creator), 0, "mesh_close (creator) failed");
    }
}

/// Zeroed selectors except `mesh`, so [`mesh_open`] joins the given id rather
/// than creating a fresh mesh.
fn create_opts_for_join(id: &CStr, nick: &CStr) -> MeshOpts {
    MeshOpts {
        mesh: id.as_ptr(),
        topic: std::ptr::null(),
        nick: nick.as_ptr(),
